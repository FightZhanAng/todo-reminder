...
**→ 2026-09-18 已修（commit `9ad640f`）**：tomcato 定的语义是「让推迟对周期任务也生效」。
...
| 14 | 到点弹通知，点「完成」 | 墨线扫地一下划过标题 → 整行淡出 → 托盘件数 -1 | ⏳ **要人手**。数据侧这一轮全验到了：点上 `firedFor` 被写入、`failed` 没触发（说明 `show()` 成功）。点不到它是因为 Windows 的 toast 是 ShellExperienceHost 的 cloaked 窗口，自动化的 `list_windows` 列不出来。**要看的就是那一眼**：墨线扫地 → 行淡出 → 托盘数字减一（这一条在 09-21 之前是坏的 —— 通知按钮绕过 `runCommand`，数据对但界面不动，已修，见 commit `deb62eb`） |
...
| 17 | 编辑它改成今天 | 出现在看板 | ✅ 2026-09-21 真机（补跑）：「以后」这本账已经存在（commit `995c101`，底栏第三个入口），从里面点开那条明天的任务 → 日期改成今天 → 「已保存」 → 回到看板出现在「今天随时」。上一轮记的 ❌ 是当时真的无处可达，那条阻塞已经解掉 |
...
| 25 | 关掉「弹出系统通知」 | 底栏显示「提醒已关闭」，点它回设置页；到点不弹通知 | ✅ 2026-09-21 真机（补跑）：底栏右下角朱砂「提醒已关闭」→ 点它直接落到设置页（「系统通知」那格是未勾状态）。到点确实不弹，而且**比期望更严**：`firedFor` 也没写，所以把提醒重新打开后，那条已过点 2 分钟的任务在 12:57:57 补发出来了 —— commit `24edfb8` 那条修正在真机上成立。复选框本身也真点了一遍：勾回去 → 底栏那条入口当场消失 → JSON `notifyEnabled = true` |
...
### 「以后」这本账（第 17 项那条阻塞补出来的入口，commit `995c101`）
...
- **通知的动作必须走命令层**（已修，`deb62eb`）。`Notifier.applyAction` 原来是
...
- **静默时段与「人不在」不再吞提醒，真机拿到了证据**（`24edfb8`）。这轮撞上了两次现场：
